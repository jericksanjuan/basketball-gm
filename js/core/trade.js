...
define(["dao", "globals", "core/league", "core/season", "core/player", "core/team", "lib/bluebird", "lib/underscore", "util/eventLog", "util/helpers", "util/random", "util/tradeHelpers"], function (dao, g, league, season, player, team, Promise, _, eventLog, helpers, random, th) {
    ...
    function genTradeScenarios(team1, teams) {
        var SCENARIOS = [
            'expiring',
            'disgruntled',
            'freespace',
        ...
        expiring = function(tx) {
            ...
            var tid, pid, tid2;
            tid = th.randomTeam(g.userTid);
            tid2 = th.randomTeam(tid);
    ...
    function _createSimTrade(tradeChance) {
        ...
        .spread(function() {
            var teams = arguments;
            return [th.randomTeam(teams), teams];
        })
        .then(function(team1, teams) {
            return genTradeScenarios(team1, teams);
        })
        .then(function(executeScene) {
            if (!executeScene) {
